...
// Legacy Audio Test GB — repository build 4
// Minimal isolated test of Display.RenderTemplate plus AudioPlayer.Play.
      ...
      directives: [
        {
          ...
          template: {
            ...
            textContent: {
              primaryText: { type: 'PlainText', text: 'Test Track' },
              secondaryText: { type: 'PlainText', text: 'Legacy Display plus AudioPlayer' },
              tertiaryText: { type: 'PlainText', text: 'English GB isolated test' }
            }
          }
        },
        {
...
exports.handler = async function handler(event) {
  ...
  console.log(JSON.stringify({
    build: 'legacy-audio-test-gb-4-strict-import',
    requestType,
    intentName,
    supportedInterfaces:
  ...
  if (requestType.startsWith('AudioPlayer.') || requestType === 'SessionEndedRequest') {
  ...
  return {
    version: '1.0',
    response: {
      outputSpeech: { type: 'PlainText', text: 'Say play test.' },
      reprompt: { outputSpeech: { type: 'PlainText', text: 'Say play test.' } },
